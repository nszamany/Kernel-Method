...
def _ch2(X, Y):
    """ Chi-squared kernel.
    """
    return np.exp(-np.sum((X - Y)**2 / (X + Y + 1e-6)))
...
def _sigmoid(X, Y, alpha=1, constant=0):
    """ Sigmoid kernel, K(X, Y) = tanh(alpha * (X . Y) + constant).
    """
    return np.tanh(alpha * np.dot(X, Y) + constant)

def sigmoid(alpha=1, constant=0):
    """ Sigmoid kernel, K(X, Y) = tanh(alpha * (X . Y) + constant).
    """
    return lambda X, Y: np.tanh(alpha * np.dot(X, Y) + constant)

def _laplacian(X, Y, sigma=1):
    """ Laplacian kernel.
    """
    return np.exp(-norm(X - Y) / sigma)

def laplacian(sigma=1):
    """ Laplacian kernel.
    """
    return lambda X, Y: np.exp(-norm(X - Y) / sigma)
